...
let handler = async (m, { conn, usedPrefix }) => {
    ...
    let str = `
 *_Total experience of_ *${name.vnmae || name.notify || name.name || ('+' + name.jid.split`@`[0])}*\n
_XP:_ *${exp}*

 *It's useless, but pss there it is xD*
`.trim()
    conn.reply(m.chat, str, m)
}
handler.help = ['inventory', 'billetera']
handler.tags = ['rpg']
handler.command = /^(billetera(entory)?|bal|level(ing)?|money|xp)$/i
