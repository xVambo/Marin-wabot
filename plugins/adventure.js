...
let handler = async (m, { conn, usedPrefix, DevMode }) => { 
    try { 
        ...
        if (global.DATABASE.data.users[m.sender].healt > 79) {
            ...
            let str = `
Your life is reduced -${healt * 1} because you have traveled up to ${pickRandom(['Jepang', 'Korea', 'Bali', 'Amerika', 'Iraq', 'Arab', 'Pakistan', 'German', 'Finlandia', 'Ke bawa dunia mimpi', 'Ujung dunia', 'Mars', 'Bulan', 'Pluto', 'Matahari', 'Hatinya dia', '...'])}  and get
*exp:* ${exp} 
*Money:* ${uang}
...
`.trim()
            ...
            if (mythic > 0) {
                   ...
                   conn.reply(m.chat, '*Congratulations you got the Rare item which is*\n' + mythic + ' Mythic Crate', m)
            }
            if (legendary > 0) {
                global.DATABASE._data.users[m.sender].legendary += legendary * 1
                conn.reply(m.chat, '*Congratulations on getting an Epic item that is*\n' + legendary + ' Legendary Crate', m)
            }
            ...
            } else conn.reply(m.chat, `You've been adventurous and exhausted, please try *${timers}* again`, m)
        } else conn.reply(m.chat, 'Minimum 80 health to be able to adventure, buy a life first by typing ' + usedPrefix + 'shop buy potion <amount>*\and type *' + usedPrefix + 'use potion <amount>*\n\n_To get free money and potions type_ *' + usedPrefix + 'claim*', m)
    } catch (e) {
        console.log(e)
