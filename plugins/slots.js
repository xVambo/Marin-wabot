...
let handler = async (m, { conn, command, args, usedPrefix, DevMode }) => { 
    ...
    if (m.chat in conn.slots) return m.reply('There are still people playing slots here, wait until it is over!!')
    else conn.slots[m.chat] = true
    try { 
        if (args.length < 1) return m.reply(`Format of use *${usedPrefix}${command} [Quantity ]*
Example *${usedPrefix}${command} 999*`)
